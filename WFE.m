function new_image = WFE(image_scaled,numfeatures)
% This the put forward unsupervised feature extraction method, called WFE
% in our paper:
...
%% Cluster bands using Kmeans
[IDX,~,~,D]=kmeans(U,numfeatures,'MaxIter',10000,'Replicates',10);
...
%%%% wighted mean
if length(b)==1
    new_image(:,i)=(image_scaled(:,b));    
else
    weight=1./D(b,i);
    weight=weight/sum(weight);
    new_image(:,i)=(image_scaled(:,b)*weight);
 end
end
